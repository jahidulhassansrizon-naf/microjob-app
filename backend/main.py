...
from autocrop_processor import (
    auto_crop_and_deskew as nid_auto_crop_and_deskew,
)

from object_adjust_processor import (
...
async def color_document_endpoint(
    ...
    try:
        ...
        return Response(
            content=
                "Color document processing failed.",
            status_code=500,
            media_type="text/plain",
        )
...
async def nid_auto_crop_endpoint(
    ...
    try:
        ...
        if not output_bytes:
            return Response(
                content=
                    "NID auto crop returned an empty image.",
                status_code=500,
                media_type="text/plain",
            )
...
@app.post("/api/object-mask")
async def object_mask_endpoint(
    file: UploadFile = File(...),
    target: str = Form(...),
):
    try:
        ...
        if not output_bytes:
            return Response(
                content=
                    "Object mask returned an empty image.",
                status_code=500,
                media_type="text/plain",
            )
        ...
        print(
            f"[Object Mask Validation] {error}"
        )
        ...
        return Response(
            content=
                "Object mask generation failed.",
            status_code=500,
            media_type="text/plain",
        )


@app.post("/api/object-adjust")
async def object_adjust_endpoint(
    file: UploadFile = File(...),
    mask: UploadFile = File(...),
    target: str = Form(...),
    brightness: float = Form(100),
    contrast: float = Form(100),
    shadow_strength: float = Form(100),
):
    try:
        target = (
            target or ""
        ...
        if not output_bytes:
            return Response(
                content=
                    "Object adjustment returned an empty image.",
                status_code=500,
                media_type="text/plain",
            )
        ...
        print(
            f"[Object Adjust Validation] {error}"
        )
        ...
        print(
            f"[Object Adjust] {error}"
        )

        return Response(
            content=
                "Object adjustment failed.",
            status_code=500,
            media_type="text/plain",
        )
...
async def apply_clothing_endpoint(
    ...
    try:
        ...
        print(
            f"[Local Clothing Fallback] {error}"
        )

        return Response(
            content=
                "Clothing processing failed.",
            status_code=500,
            media_type="text/plain",
        )
