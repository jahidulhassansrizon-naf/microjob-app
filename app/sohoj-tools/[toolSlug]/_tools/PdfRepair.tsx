  ...
  return (
    <div className="w-full space-y-5 text-gray-800 font-sans">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500">
        ...
        <span>PDF Tools</span>
        <span>/</span>
        <span className="font-semibold text-gray-900">PDF Repair</span>
      </div>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-gray-900">PDF Repair</h1>
              <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                Free
              </span>
            </div>
            <p className="text-xs text-gray-500 font-medium mt-0.5">
              Fix corrupted or unreadable PDFs with a three-step repair engine
              in your browser.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        {/* Left Side: Upload & Explanation Box */}
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              ...
              <label className="inline-flex items-center gap-1 text-[11px] font-bold text-red-500 bg-red-50 hover:bg-red-100 px-2 py-0.5 rounded-md cursor-pointer transition-colors border border-red-100">
                <Plus size={12} />
                <span>Add</span>
                <input
                  type="file"
            ...
            {!selectedFile ? (
              <label className="border border-dashed border-red-300 bg-red-50/20 hover:bg-red-50/40 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all group min-h-[170px]">
                <div className="w-9 h-9 bg-red-100/80 text-red-500 rounded-xl flex items-center justify-center mb-2 group-hover:scale-105 transition-transform">
                  <Upload size={18} />
                </div>
          ...
          {/* How repair works card */}
          <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-xs space-y-2">
            <h3 className="text-xs font-bold text-gray-900">
              How repair works
            </h3>
            <ul className="text-[11px] text-gray-500 space-y-1.5 leading-snug">
              <li>
                <strong className="text-gray-700 font-semibold">Step 1:</strong>{" "}
                Rewrite the PDF structure with a clean save.
              </li>
              <li>
                <strong className="text-gray-700 font-semibold">Step 2:</strong>{" "}
                Rebuild the file by copying pages into a new document.
              </li>
              <li>
                <strong className="text-gray-700 font-semibold">Step 3:</strong>{" "}
                Render each page and assemble a new PDF (works on heavily
                damaged files).
              </li>
            </ul>
          </div>
      ...
      <div className="bg-white p-3.5 rounded-2xl border border-gray-100 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-xs font-medium text-gray-500">
          ...
          <span>
            {selectedFile
              ? `Ready to repair: ${selectedFile.name}`
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
        ...
        {showHowToUse && (
          <div className="px-5 pb-4 text-xs text-gray-500 leading-relaxed border-t border-gray-50 pt-3">
            Upload your damaged or corrupted PDF file, click "Repair PDF", and
            download the fixed document once processed.
          </div>
        )}
      </div>
